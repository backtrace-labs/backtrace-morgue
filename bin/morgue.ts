...
const flamegraph = path.join(__dirname, '..', 'assets', 'flamegraph.pl');

let callstackError = false;
const range_start: number | null = null;
const range_stop: number | null = null;
let endpoint: string;
...
function tenantURL(config: any, tn: any): string {
   ...
   * in this case.
   */
  if (!config.config.universe) return config.endpoint;
...
function coronerLog(argv: any, config: any): any {
  ...
  if (action === 'list') {
    ...
    if (argv.universe) q.form.universe = argv.universe;
...
function coronerLatency(argv: any, config: any): any {
  ...
  if (action === 'list') {
    coroner.control2(
      ...
      (error, rp) => {
        ...
        for (let i = 0; i < hs.length; i++) {
          if (argv._[2] && hs[i].name.match(argv._[2]) === null) continue;
    ...
    if (argv.samples) samples = argv.samples;
...
function callstackPrint(cs: any): void {
  ...
  if (!ARGV.verbose && !ARGV.l) {
    const options: CallstackOptions = {};
    const label = new Callstack(frames);

    if (ARGV.collapse) options.dynamic = true;
    if (ARGV.suffix) {
