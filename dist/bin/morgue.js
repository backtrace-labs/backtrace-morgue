...
var CoronerClient = require('../lib/coroner.js');
var crdb = require('../lib/crdb.js');
...
var range_stop = null;
var reverse = 1;
var configDir = path.join(os.homedir(), ".morgue");
var configFile = path.join(configDir, "current.json");
...
function printSamples(requests, samples, start, stop, concurrency) {
  ...
  process.stdout.write(printf("# %12s %12s %12s %12s %12s %12s %12s\n", "Concurrency", "Requests", "Time", "Minimum", "Average", "Maximum", "Throughput").grey);
  process.stdout.write(printf("  %12d %12ld %12f %12ld %12ld %12ld %12ld\n", concurrency, requests, (stop - start) / 1000000, minimum, sum, maximum, tps));
  return;
}
...
function saveConfig(coroner, callback) {
  makeConfigDir(function (err) {
    ...
    if (coroner.config.endpoints.post) {
      ...
      config.submissionEndpoint = ep[0].protocol + '://' + fu.hostname + ':' + ep[0].port + '/post';
    }
  ...
  if (argv.smr) {
    coroner.control({ 'action': 'graceperiod' }, function (error, r) {
      if (error) {
        ...
        if (error === 'invalid token') message = message + ': try logging in again.';
...
function coronerGet(argv, config) {
  ...
  if (argv.resource) rf = argv.resource;
...
function coronerDescribe(argv, config) {
  ...
  coroner.describe(universe, project, function (error, result) {
    ...
    for (i = 0; i < cd.length; i++) {
      ...
      if (filter && _it.name.match(filter) === null) continue;

      name = printf("%*s", _it.name, ml);
      if (_it.custom === true) {
        process.stdout.write(name.blue + ': ' + _it.description);
      } else {
        process.stdout.write(name.yellow + ': ' + _it.description);
      }
      if (_it.format) process.stdout.write(' ['.grey + _it.format.grey + ']'.grey);
      ...
      (function () {
        ...
        coroner.put(files[bind].body, {
          ...
          if (submitted === files.length) {
            if (success === files.length) console.log('Success'.blue);

            if (!argv.benchmark) process.exit(0);
          }
        });
      })();
    }
...
function coronerList(argv, config) {
  ...
  if (argv.filter) {
    ...
    for (i = 0; i < argv.filter.length; i++) {
      ...
      if (!query.filter[0][r[0]]) query.filter[0][r[0]] = [];
      query.filter[0][r[0]].push([r[1], r[2]]);
    }
  }

  if (!query.filter[0].timestamp) query.filter[0].timestamp = [];
  ...
  if (argv.template === 'select') {} else if (argv.select) {
    if (!query.select) query.select = [];

    if (Array.isArray(argv.select) === true) {
      for (var _i = 0; _i < argv.select.length; _i++) {
        query.select.push(argv.select[_i]);
      }
    } else {
  ...
  if (d_age) {
    ...
    query.filter[0].timestamp = [['at-least', oldest]];
  ...
  if (argv.tail) fold(query, argv.tail, 'tail', unaryPrint);
  if (argv.head) fold(query, argv.head, 'head', unaryPrint);
  if (argv.object) fold(query, argv.object, 'object', noFormatPrint);
  if (argv.histogram) fold(query, argv.histogram, 'histogram', histogramPrint);
  if (argv.unique) fold(query, argv.unique, 'unique', unaryPrint);
  if (argv.sum) fold(query, argv.sum, 'sum', unaryPrint);
  if (argv.quantize) fold(query, argv.quantize, 'bin', binPrint);
  if (argv.bin) fold(query, argv.bin, 'bin', binPrint);
  if (argv.range) fold(query, argv.range, 'range', rangePrint);

  if (argv.query) {
    var pp = JSON.stringify(query);

    console.log(pp);
    if (!argv.raw) process.exit(0);
  }
  ...
  if (argv.benchmark) {
    ...
    for (i = 0; i < concurrency; i++) {
      (function queryPr() {
        ...
        coroner.query(universe, project, query, function (err, result) {
          ...
          if (--n_samples == 0) {
            printSamples(requests, samples, start, process.hrtime(), concurrency);
            process.exit(0);
          }
    ...
    coroner.query(universe, project, query, function (err, result) {
      ...
      coronerPrint(query, rp, result.response, argv.sort, argv.limit);

      var footer = result._.user + ': ' + result._.universe + '/' + result._.project + ' as of ' + d_age + ' ago [' + result._.latency + ']';
...
function fieldFormat(st, format) {
  var rd = {
    'memory_address': function memory_address() {
      return printf("%#lx", st);
    },
    'kilobytes': function kilobytes() {
      return st + ' kB';
    },
    'megabytes': function megabytes() {
      return st + ' MB';
    },
    'gigabytes': function gigabytes() {
      return st + ' GB';
    },
...
function binPrint(field, factor, ff) {
  ...
  for (i = 0; i < field.length; i++) {
    ...
    label = printf(format, fieldFormat(field[i][0], ff), fieldFormat(field[i][1], ff));
...
function objectPrint(g, object, columns, fields) {
  ...
  if (object.length) {
    ...
    for (i = 0; i < object.length; i++) {
      ...
      if (ob.timestamp) {
        process.stdout.write(new Date(ob.timestamp * 1000) + '     ' + ta.ago(ob.timestamp * 1000).bold + '\n');
      } else {
        process.stdout.write('\n');
      }
  ...
  if (timestamp_range) {
    ...
    if (timestamp_range[0] !== timestamp_range[1]) console.log('      ' + stop);
  }

  if (object.count) console.log('Occurrences: '.yellow.bold + object.count);
  ...
  for (field in object) {
     ...
     */
    if (field.indexOf('timestamp') > -1 && (field.indexOf('bin(') > -1 || field.indexOf('range(') > -1)) {
      continue;
    }
    ...
    if (columns[match](object[field], field.label, fields[field]) === false) console.log(object[field]);
  }
}

function range_compare(a, b, sort) {
  return reverse * ((a[1][sort][1] < b[1][sort][1]) - (a[1][sort][1] > b[1][sort][1]));
}

function unique_compare(a, b, sort) {
  return reverse * ((a[1][sort][0] < b[1][sort][0]) - (a[1][sort][0] > b[1][sort][0]));
}
...
function coronerPrint(query, rp, raw, sort, limit, columns) {
  ...
  if (sort) {
    ...
    if (limit && limit < length) length = limit;
