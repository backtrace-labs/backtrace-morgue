...
CoronerClient.prototype.http_get = function(path, params, opts, callback) {
  ...
  function onResponse(err, resp, body) {
    ...
    if (self.debug) {
      debug_response(resp, body.toString('utf8'));
    }
    callback(null, Object.assign({bodyData: body}, resp));
  }
...
CoronerClient.prototype.post = function(path, params, body, opt, callback) {
  ...
  if (self.debug) {
    console.error("POST " + options.uri);
...
CoronerClient.prototype.post_form = function(dumpfile, attachments, params, callback) {
  ...
  var fullPath = options.protocol  + '//' + options.host + '/api/post' + '?' +
    qs.stringify(params);
  if (this.debug) {
  ...
  var req = request.post(fullPath, function onResponse(err, httpResponse, body) {
    ...
    if (self.debug) {
      debug_response(httpResponse, body);
    }
    var json;
    try {
  ...
  if (attachments) {
    ...
    attachments.forEach(function(aobj) {
      ...
      if (self.debug) {
        console.log('Attachment: ', JSON.stringify(aobj, null, 4));
      }
      name = "attachment_" + path.basename(aobj.filename);
      form.append(name, fs.createReadStream(aobj.filename), aobj);
    });
  }
};
