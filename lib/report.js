...
function href(st, event) {
  var base = config.coroner.endpoint;

  if (base[base.length - 1] != '/')
    base += '/';

  var url = base + 'dashboard/' + config.universe + '/project/' + config.project + '?granularity=week';
  var ev = events[event];
...
class Report {
  constructor(coroner, universe, project, options) {
    ...
    if (options) {
      config.top = options.top ? options.top : config.top;
      config.feed = options.feed ? options.feed : config.feed;
    }
  }

  histograms(cb) {
    var self = this;
    var queries = config.top;
    var results = [];
  ...
  events(cb) {
    ...
    self.coroner.query(self.universe, self.project, query, function(error, result) {
      ...
      for (var i = 0; i < results.length; i++) {
        ...
        if (results[i].timestamp > lastWeek && results[i].timestamp <= yesterday) {
...
` + lastWeek + ` &mdash; ` + now + `
<br><small>` + href('Click here to login', 'login') + `</small>
<br>
</tr></table>
</div></div>
...
`;
    ...
    self.events(function(result) {
      function weekHistogram(title, h, sum, scale_user) {
        ...
        /* Rotate days so current day is first. */
        days.rotate(today);
      ...
      self.histograms(function(rc) {
        ...
        self.activity(null, function(result) {
          ...
          fs.appendFileSync(path,
            ...
            '<div class="container" style="padding: 10px 20px; max-width: 600px; font-weight: 300; margin: 0 auto; text-align: left">' +
            (config.feed.limit > 0 ? '<h3>Recent Activity</h3>' : '') +
            buffer +
            "<hr><small>Report generated " + new Date() + "</small>"
          );
        });
      });
